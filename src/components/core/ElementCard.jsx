...
export default function ElementCard({ title, subtitle, children, controls, copyData, focused, onFocus, layout = "split" }) {
  ...
  return (
    <div onClick={onFocus} style={{
      ...
      <div style={layout === "split"
        ...
        <div style={layout === "split" ? { width: UI_TOKENS.split.controlWidth, flexShrink: 0 } : undefined}>{controlContent}</div>
      </div>
    </div>
  );
}

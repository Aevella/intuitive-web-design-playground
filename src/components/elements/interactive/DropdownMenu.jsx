...
import { monoFont, noteTextStyle } from "../../../utils/stylePresets";
...
const Component = memo(({ focused, onFocus }) => {
  ...
  return (
    <ElementCard
      ...
      focused={focused}
      onFocus={onFocus}
      copyData={{ css, short: `motion:${p.motion} · duration:${p.duration.toFixed(2)}s · max-h:${p.maxHeight}px` }}
    >
      ...
      <div>
        ...
        <ParamSlider label="最大高度 max-height" value={p.maxHeight} min={90} max={240} unit="px" onChange={(v) => setP((s) => ({ ...s, maxHeight: v }))} accent={accent} snaps={[120, 170, 210]} />
